...
from .registries import ModelRegistry, ToolRegistry, get_model_registry, get_tool_registry
...
class UniversalEventProcessor:
    ...
    async def process_event(self, event: Event) -> List[Event]:
        ...
        try:
            ...
            logging.info(f"Processed event {event.type} -> {len(output_events)} output events")
...
class EventMetrics:
    ...
    async def record_event(self, event: Event, output_events: List[Event], processing_time: float):
