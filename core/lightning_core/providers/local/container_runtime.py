...
class DockerContainerRuntime(ContainerRuntime):
    ...
    def _config_to_docker_params(self, config: ContainerConfig) -> Dict[str, Any]:
            ...
            if config.resources.gpu_count > 0:
                params["device_requests"] = [
                    docker.types.DeviceRequest(
                        count=config.resources.gpu_count, capabilities=[["gpu"]]
                    )
                ]
        ...
        params["restart_policy"] = restart_policies.get(
            config.restart_policy, {"Name": "on-failure", "MaximumRetryCount": 3}
        )
    ...
    def _docker_to_container(
        ...
        if docker_container.attrs.get("NetworkSettings", {}).get("Networks"):
            for network in docker_container.attrs["NetworkSettings"][
                "Networks"
            ].values():
        ...
        container = Container(
            ...
            metadata={"docker_status": docker_container.status, "docker_attrs": attrs},
        )

        return container

    def _extract_config_from_docker(
        ...
        config = ContainerConfig(
            ...
            environment_variables=dict(
                env.split("=", 1) for env in config_data.get("Env", []) if "=" in env
            ),
    ...
    async def create_container(
        self, config: ContainerConfig, wait_for_ready: bool = True
    ) -> Container:
        ...
        try:
            ...
            docker_container = await loop.run_in_executor(
                None, lambda: self._client.containers.run(**params)
            )

            logger.info(
                f"Created container {config.name} with ID {docker_container.id}"
            )
            ...
            docker_container = await loop.run_in_executor(
                None, lambda: self._client.containers.get(docker_container.id)
            )
    ...
    async def _wait_for_ready(self, container_id: str, timeout: int = 30) -> None:
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout:
            try:
                container = await loop.run_in_executor(
                    None, lambda: self._client.containers.get(container_id)
                )
        ...
        raise TimeoutError(
            f"Container {container_id} failed to become ready within {timeout} seconds"
        )
    ...
    async def list_containers(
        ...
        docker_containers = await loop.run_in_executor(
            None, lambda: self._client.containers.list(all=True, filters=filters)
        )
    ...
    async def stop_container(
        ...
        try:
            ...
            await loop.run_in_executor(
                None, lambda: docker_container.stop(timeout=timeout_seconds)
            )
    ...
    async def remove_container(self, container_id: str, force: bool = False) -> None:
        ...
        try:
            ...
            await loop.run_in_executor(
                None, lambda: docker_container.remove(force=force)
            )
    ...
    async def get_container_logs(
        ...
        try:
            ...
            logs = await loop.run_in_executor(
                None, lambda: docker_container.logs(**kwargs)
            )
    ...
    async def exec_command(
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, lambda: docker_container.exec_run(**exec_kwargs)
            )
    ...
    async def pull_image(
        self, image: str, auth_config: Optional[Dict[str, str]] = None
    ) -> None:
        ...
        try:
            await loop.run_in_executor(
                None, lambda: self._client.images.pull(image, auth_config=auth_config)
            )
    ...
    async def push_image(
        ...
        try:
            await loop.run_in_executor(
                None, lambda: self._client.images.push(image, auth_config=auth_config)
            )
    ...
    async def wait_for_container(
        self, container_id: str, timeout_seconds: Optional[int] = None
    ) -> int:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, lambda: docker_container.wait(timeout=timeout_seconds)
            )
    ...
    async def get_container_stats(self, container_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = await loop.run_in_executor(
                None, lambda: docker_container.stats(stream=False)
            )
            ...
            return {
                ...
                "block_read_bytes": sum(
                    io.get("value", 0)
                    for io in stats.get("blkio_stats", {}).get(
