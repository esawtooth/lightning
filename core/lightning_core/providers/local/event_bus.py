...
class LocalEventBus(EventBus):
    ...
    async def _invoke_handler(
        ...
        try:
            ...
            logger.error(f"Error in handler {subscription.subscription_id} for event {event.id}: {e}")
            ...
            if retry_count < self._max_retries:
                ...
                logger.error(f"Event {event.id} moved to dead letter queue after {retry_count} retries")