    ...
    async def _execute_external(self, context: FunctionContext) -> FunctionResponse:
        ...
        if self.config.runtime == RuntimeType.PYTHON:
            ...
            raise NotImplementedError(
                f"Runtime {self.config.runtime} not supported in local mode"
            )
...
""".format(
            ...
            # Prepare environment
            env = {**self.config.environment_variables, "PYTHONPATH": str(work_dir)}
...
class LocalServerlessRuntime(ServerlessRuntime):
    ...
    async def deploy_function(
        ...
        local_function = LocalFunction(
            function_id=function_id, config=config, handler=handler, code_path=code_path
        )
    ...
    async def invoke_function(
        ...
        if async_invoke:
            ...
            return FunctionResponse(
                status_code=202, body={"message": "Function invoked asynchronously"}
            )
        else:
    ...
    async def list_functions(
        ...
        for function_id, local_function in self._functions.items():
            ...
            if labels:
                if not all(
                    local_function.config.labels.get(k) == v for k, v in labels.items()
                ):
                    continue
            ...
            functions.append(
                {
                    ...
                    "last_invocation": (
                        local_function.last_invocation.isoformat()
                        if local_function.last_invocation
                        else None
                    ),
    ...
    async def set_function_concurrency(
        ...
        logger.info(
            f"Set concurrency for {function_id} to {reserved_concurrency} (no-op in local mode)"
        )
    ...
    async def unregister_event_source(self, event_source_id: str) -> None:
        ...
        # In local mode, this is a no-op
        logger.info(
            f"Unregistered event source {event_source_id} (no-op in local mode)"
        )
