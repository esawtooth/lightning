...
from azure.servicebus.exceptions import MessageNotFoundError, ServiceBusError
...
class ServiceBusEventBus(EventBus):
        ...
        **kwargs: Any,
        ...
        if connection_string:
            self._client = ServiceBusClient.from_connection_string(
                connection_string, logging_enable=kwargs.get("logging_enable", False)
            )
            ...
            if conn_str:
                self._client = ServiceBusClient.from_connection_string(
                    conn_str, logging_enable=kwargs.get("logging_enable", False)
                )
            else:
    ...
    async def create_topic(self, topic_name: str) -> None:
        ...
        logger.warning(
            f"Queue creation not implemented. Ensure {topic_name} exists in Service Bus namespace"
        )
    ...
    async def delete_topic(self, topic_name: str) -> None:
        ...
        logger.warning(
            f"Queue deletion not implemented. Delete {topic_name} through Azure Portal"
        )
    ...
    async def reprocess_dead_letter_event(
        ...
        async with self._client.get_queue_receiver(
            queue_name=dead_letter_queue, max_message_count=100
        ) as receiver:
            # Find the message
            messages = await receiver.receive_messages(
                max_message_count=100, max_wait_time=self._max_wait_time
            )
    ...
    async def _start_queue_processor(self, queue_name: str) -> None:
        ...
        if queue_name not in self._receivers:
            self._receivers[queue_name] = self._client.get_queue_receiver(
                queue_name=queue_name, max_message_count=self._max_message_count
            )
