...
class CosmosDocumentStore(DocumentStore[T]):
    ...
    async def update(self, document: T) -> T:
        ...
        try:
            ...
            access_condition = {"type": "IfMatch", "condition": document.etag} if document.etag else None
    ...
    def get_document_store(self, container_name: str, document_type: Type[T]) -> DocumentStore[T]:
        ...
        if container_name not in self._stores:
            if container_name not in self._containers:
                raise ValueError(f"Container {container_name} does not exist. Call create_container_if_not_exists first.")
