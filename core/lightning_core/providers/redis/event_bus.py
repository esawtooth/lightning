...
class RedisEventBus(EventBus):
    ...
    async def subscribe(
        ...
        logger.info(f"Created subscription {subscription_id} for {event_type} on channel {channel}")
        ...
        for subscription in matching_subscriptions:
            if self._matches_filter(event, subscription.filter_expression):
                try:
                    ...
                    logger.debug(f"Successfully processed event {event.id} with handler {subscription.subscription_id}")
                except Exception as e:
                    logger.error(f"Handler {subscription.subscription_id} failed for event {event.id}: {e}")
                    
                    # Add to dead letter queue
                    dead_letter_key = f"{self._dead_letter_prefix}{event_type}:{event.id}"
