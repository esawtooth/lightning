...
class AccessControl:
    ...
    user_permissions: Dict[str, bool] = field(default_factory=dict)  # user_id -> enabled
...
class UnifiedToolRegistry:
    ...
    def list_tools(
        ...
        if access_scope:
            tools = [t for t in tools if t.access_control.is_accessible_to(access_scope, user_id)]
        
        return tools
    
    def get_tools_for_scope(self, scope: AccessScope, user_id: Optional[str] = None) -> List[ToolSpec]:
    ...
    def get_agent_tools(self, agent_name: str, user_id: Optional[str] = None) -> List[ToolSpec]:
        ...
        try:
            ...
            for tool_id, tool_data in planner_tools.items():
                if tool_id not in self.tools:
                    ...
                    # Update existing tool with planner data
                    existing_tool = self.tools[tool_id]
                    existing_tool.inputs = tool_data.get("inputs", {})
                    existing_tool.produces = tool_data.get("produces", [])
                    if not existing_tool.description:
        ...
        try:
            for tool_id, vextir_tool in vextir_registry.tools.items():
                if tool_id not in self.tools:
                    ...
                    existing_tool.resource_requirements = vextir_tool.resource_requirements
    ...
    def _load_default_tools(self) -> None:
        ...
        conseil_agent = ToolSpec(
            ...
            capabilities=["task_execution", "bash_access", "context_access", "reasoning"],
        ...
        llm_general = ToolSpec(
            ...
            description="Call an LLM {one of gpt-4o, o3, o4-mini, claude sonnet 4} with the provided system and user prompts",
            tool_type=ToolType.FUNCTION,
            inputs={"system_prompt": "string", "user_prompt": "string", "model": "string"},
            produces=["event.llm.response"],
        ...
        # Communication tools - Available to agents only
        teams_chat = ToolSpec(
            id="chat.sendTeamsMessage",
            name="Teams Message",
            description="Send Teams chat message",
        ...
        web_search = ToolSpec(
            ...
            inputs={"query": "string", "max_results": "string", "source_filter": "string"},
...
def get_tool_registry() -> UnifiedToolRegistry:
    ...
    if _global_registry is None:
        ...
        try:
            ...
            planner_json_path = Path(__file__).parent.parent / "planner" / "registry.tools.json"
