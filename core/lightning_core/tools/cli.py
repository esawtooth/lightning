...
from .registry import get_tool_registry, ToolType, AccessScope
...
Examples:
  ...
  python -m lightning_core.tools.cli sync-planner
    ...
    type_parser.add_argument("type", help="Tool type (native, mcp_server, api, function, agent)")
    
    # List by capability
    cap_parser = subparsers.add_parser("list-by-capability", help="List tools by capability")
    cap_parser.add_argument("capability", help="Capability name (e.g., web_search, email_send)")
    
    # Show tool details
    show_parser = subparsers.add_parser("show-tool", help="Show detailed tool information")
    show_parser.add_argument("tool_id", help="Tool ID to show details for")
    
    # Compare registries
    subparsers.add_parser("compare", help="Compare unified registry vs planner registry")
