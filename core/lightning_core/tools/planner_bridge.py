...
from .registry import AccessScope, get_tool_registry
...
class PlannerToolBridge:
    ...
    def load(self, path: Path = None, user_id: str = None) -> Dict[str, Any]:
        ...
        return {tool.id: tool.to_planner_format() for tool in planner_tools}
    ...
    async def register_plan_as_application(
        self, plan_json: Dict[str, Any], user_id: str = None
    ) -> str:
        """Register a plan as a first-class application in Vextir OS"""
        return await self._plan_manager.register_plan_from_json(
            plan_json, user_id or "system"
        )
