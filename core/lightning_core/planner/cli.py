...
from .validator import validate_plan_new, run_validations_parallel, PlanValidationError
...
def call_planner_llm_with_debug(instruction: str, registry_subset: Dict[str, Any], verbose: bool = False) -> Dict[str, Any]:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            logger.info(f"   Validation results: {len(passed_validators)}/{len(results)} passed")
            ...
            if len(failed_validators) == 0:
                ...
                error_summary = "\n".join([f"- {r.validator_name}: {r.error_message}" for r in failed_validators])
    ...
    raise RuntimeError(f"Planner could not produce a valid plan in {max_retries} attempts.")


def generate_plan(instruction: str, output_file: Optional[str] = None, verbose: bool = False) -> Dict[str, Any]:
...
def visualize_plan(plan_file: str, output_file: Optional[str] = None, format: str = "png", verbose: bool = False):
    ...
    try:
        ...
        for event in plan['events']:
            name = event['name']
            color = event_colors['external'] if name in external_events else event_colors['internal']
        ...
        for step in plan['steps']:
            for emitted_event in step.get('emits', []):
                if emitted_event not in [evt['name'] for evt in plan['events']]:
                    dot.node(emitted_event, emitted_event, fillcolor=event_colors['internal'])
        ...
        for step in plan['steps']:
            ...
            dot.node(step_name, label, fillcolor='#fff3e0', shape='box')  # Light orange for steps
...
def generate_mermaid_diagram(plan_file: str, output_file: Optional[str] = None, verbose: bool = False):
...
def register_plan_application(plan_file: str, user_id: str, verbose: bool = False) -> bool:
...
def unregister_plan_application(plan_id: str, user_id: str, verbose: bool = False) -> bool:
...
Examples:
  ...
  lightning-planner list-events
    ...
    # Generate command
    generate_parser = subparsers.add_parser('generate', help='Generate a plan from instruction')
    generate_parser.add_argument('instruction', help='Natural language instruction for the plan')
    generate_parser.add_argument('-o', '--output', help='Output file for the generated plan')
    ...
    # Visualize command
    visualize_parser = subparsers.add_parser('visualize', help='Generate visual representation of a plan')
    visualize_parser.add_argument('plan_file', help='Path to the plan JSON file')
    visualize_parser.add_argument('-o', '--output', help='Output file for the visualization')
    visualize_parser.add_argument('-f', '--format', default='png', choices=['png', 'svg', 'pdf'], 
                                help='Output format (default: png)')
    
    # Mermaid command
    mermaid_parser = subparsers.add_parser('mermaid', help='Generate Mermaid diagram of a plan')
    mermaid_parser.add_argument('plan_file', help='Path to the plan JSON file')
    mermaid_parser.add_argument('-o', '--output', help='Output file for the Mermaid diagram')
    ...
    execute_parser.add_argument('-u', '--user-id', default='default', help='User ID for plan execution')
    
    # Setup command
    setup_parser = subparsers.add_parser('setup', help='Set up a plan without executing it')
    setup_parser.add_argument('plan_file', help='Path to the plan JSON file')
    setup_parser.add_argument('-u', '--user-id', default='default', help='User ID for plan setup')
    ...
    register_parser = subparsers.add_parser('register-app', help='Register a plan as a first-class application')
    register_parser.add_argument('plan_file', help='Path to the plan JSON file')
    register_parser.add_argument('-u', '--user-id', default='default', help='User ID for plan registration')
    
    unregister_parser = subparsers.add_parser('unregister-app', help='Unregister a plan application')
    unregister_parser.add_argument('plan_id', help='ID of the plan to unregister')
    unregister_parser.add_argument('-u', '--user-id', default='default', help='User ID for plan unregistration')
    
    subparsers.add_parser('list-apps', help='List all registered plan applications')
    
    show_app_parser = subparsers.add_parser('show-app', help='Show details of a specific plan application')
    ...
    try:
        if args.command == 'generate':
            ...
            success = visualize_plan(args.plan_file, args.output, args.format, args.verbose)
            return 0 if success else 1
            
        elif args.command == 'mermaid':
            success = generate_mermaid_diagram(args.plan_file, args.output, args.verbose)
            return 0 if success else 1
            ...
            success = register_plan_application(args.plan_file, args.user_id, args.verbose)
            return 0 if success else 1
            
        elif args.command == 'unregister-app':
            success = unregister_plan_application(args.plan_id, args.user_id, args.verbose)
            return 0 if success else 1
