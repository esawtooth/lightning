...
class PetriNetValidator(Validator):
    ...
    def _validate_impl(self, plan: Dict[str, Any]) -> None:
        try:
            net, initial_marking, final_marking = self._build_petri_net_with_markings(plan)
    ...
    def _get_debug_info(self, net: PetriNet, initial_marking: Marking, final_marking: Marking, plan: Dict[str, Any]) -> str:
        ...
        info.append(f"Arcs: {len([arc for place in net.places for arc in place.in_arcs]) + len([arc for place in net.places for arc in place.out_arcs])}")
        ...
        for place in net.places:
            ...
            info.append(f"  {place.name}: {place_type} (in:{in_arcs}, out:{out_arcs}, init:{initial_tokens}, final:{final_tokens})")
        ...
        for transition in net.transitions:
            ...
            info.append(f"  {transition.name}: in:{in_arcs} {input_places} -> out:{out_arcs} {output_places}")
        
        info.append(f"\n--- MARKINGS ---")
        info.append(f"Initial marking: {[(place.name, tokens) for place, tokens in initial_marking.items()]}")
        info.append(f"Final marking: {[(place.name, tokens) for place, tokens in final_marking.items()]}")
        
        # Analyze workflow net properties
        info.append(f"\n--- WORKFLOW NET ANALYSIS ---")
        source_places = [p for p in net.places if len(p.in_arcs) == 0 and len(p.out_arcs) > 0]
        sink_places = [p for p in net.places if len(p.in_arcs) > 0 and len(p.out_arcs) == 0]
        isolated_places = [p for p in net.places if len(p.in_arcs) == 0 and len(p.out_arcs) == 0]
        
        info.append(f"Source places (no input, has output): {[p.name for p in source_places]}")
        info.append(f"Sink places (has input, no output): {[p.name for p in sink_places]}")
        info.append(f"Isolated places (no input, no output): {[p.name for p in isolated_places]}")
        
        # Check workflow net requirements
        info.append(f"\n--- WORKFLOW NET REQUIREMENTS ---")
        info.append(f"✓ Exactly 1 source place required: {len(source_places) == 1} (found {len(source_places)})")
        info.append(f"✓ Exactly 1 sink place required: {len(sink_places) == 1} (found {len(sink_places)})")
        info.append(f"✓ No isolated places allowed: {len(isolated_places) == 0} (found {len(isolated_places)})")
        ...
        orphaned_events = set(evt["name"] for evt in plan["events"]) - all_consumed_events - all_emitted_events
        if orphaned_events:
            info.append(f"⚠️  Orphaned events (not consumed or emitted): {sorted(orphaned_events)}")
    ...
    def _build_petri_net_with_markings(self, plan: Dict[str, Any]) -> Tuple[PetriNet, Marking, Marking]:
        """Build Petri net from plan with initial and final markings"""
        net = PetriNet(plan["plan_name"])
        
        # Create places for all events
        places = {}
        for evt in plan["events"]:
            name = evt["name"]
        ...
        # If no final events found, use a simple heuristic:
        # mark the last emitted events as final
        if not final_marking:
            # Find events emitted by the last steps (those that don't trigger other steps)
            triggering_events = set()
            for step in plan["steps"]:
                triggering_events.update(step["on"])
            
            for step in plan["steps"]:
                for emitted_event in step.get("emits", []):
                    if emitted_event not in triggering_events and emitted_event in places:
                        final_marking[places[emitted_event]] = 1
