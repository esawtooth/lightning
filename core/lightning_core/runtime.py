...
from .abstractions import (
    ...
    StorageProvider, EventBus, ContainerRuntime, ServerlessRuntime,
    Document, EventMessage, EventHandler
)
...
class LightningRuntime:
    ...
    def container_runtime(self) -> ContainerRuntime:
        ...
        if not self._container_runtime:
            self._container_runtime = ProviderFactory.create_container_runtime(self.config)
    ...
    def serverless(self) -> ServerlessRuntime:
        ...
        if not self._serverless_runtime:
            self._serverless_runtime = ProviderFactory.create_serverless_runtime(self.config)
...
async def initialize_runtime(config: Optional[RuntimeConfig] = None) -> LightningRuntime:
    """
