...
@driver("plan_executor", DriverType.TOOL,
...
class PlanExecutorDriver(ToolDriver):
    ...
    async def handle_event(self, event: Event) -> List[Event]:
        ...
        if event.type == "plan.execute":
            ...
            if plan_id and plan_id in self.active_plans:
                trigger_events = await self._trigger_plan(plan_id, trigger_event, event.user_id)
            ...
            plan_data = event.metadata.get("plan")
            if plan_data:
                register_events = await self._register_plan_as_application(plan_data, event.user_id)
            ...
            plan_id = event.metadata.get("plan_id")
            if plan_id:
                unregister_events = await self._unregister_plan_application(plan_id, event.user_id)
                output_events.extend(unregister_events)
        
        return output_events
    
    async def _execute_plan(self, plan_data: Dict[str, Any], user_id: str) -> List[Event]:
        ...
        try:
            ...
            external_events = [
                evt for evt in plan_data.get("events", [])
    ...
    async def _handle_cron_configuration(self, event: Event) -> List[Event]:
        ...
        try:
            ...
            if plan_id and cron_expression:
                ...
                logging.info(f"Configured cron job for plan {plan_id}: {cron_expression}")
    ...
    async def _trigger_plan(self, plan_id: str, trigger_event: str, user_id: str) -> List[Event]:
        ...
        try:
            ...
            logging.info(f"Triggered {len(triggered_steps)} steps for plan {plan_id} with event {trigger_event}")
    ...
    async def _register_plan_as_application(self, plan_data: Dict[str, Any], user_id: str) -> List[Event]:
    ...
    async def _unregister_plan_application(self, plan_id: str, user_id: str) -> List[Event]:
        """Unregister a plan application"""
        output_events = []
...
async def register_plan_execution_drivers():
    ...
    # Register drivers
    drivers = [
        (PlanExecutorDriver._vextir_manifest, PlanExecutorDriver)
    ]
...
class PlanApplicationManager:
    ...
    async def register_plan_from_json(self, plan_json: Dict[str, Any], user_id: str = "system") -> str:
