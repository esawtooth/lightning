...
from ..abstractions.event_bus import EventBus
from ..abstractions.factory import get_provider_factory, get_runtime_config
...
class InstructionEngineDriver(AgentDriver):
    ...
    def __init__(
        ...
        # Initialize provider-agnostic storage and event bus
        factory = get_provider_factory()
        runtime_config = get_runtime_config()
        self._storage = factory.create_storage_provider(runtime_config)
        self._event_bus = factory.create_event_bus(runtime_config)

        # Collection names
        self.instruction_collection = (
            config.get("instruction_collection", "instructions")
            if config
            else "instructions"
        )
...
"""
    ...
    async def handle_event(self, event: Event) -> List[Event]:
        ...
        if not instructions:
            logging.info(
                f"No instructions found for user {event.user_id}, discarding event {event.type}"
            )
            return output_events
        ...
        for instruction in instructions:
            if await self._match_event_to_instruction(event, instruction):
                matched_count += 1
                logging.info(
                    f"Event {event.type} matched instruction {instruction['name']} for user {event.user_id}"
                )

                action_events = await self._execute_instruction_action(
                    event, instruction
                )
        ...
        if matched_count == 0:
            logging.info(
                f"Event {event.type} did not match any instructions for user {event.user_id}, discarding"
            )
        else:
            logging.info(
                f"Event {event.type} matched {matched_count} instructions for user {event.user_id}"
            )
    ...
    async def _get_user_instructions(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            query = {
                ...
                "collection": self.instruction_collection,
            }
            items = await self._storage.query_documents(query)
            return items
        except Exception as e:
            logging.error(f"Failed to get instructions for user {user_id}: {e}")
            return []

    async def _match_event_to_instruction(
        self, event: Event, instruction: Dict[str, Any]
    ) -> bool:
    ...
    async def _execute_instruction_action(
        self, event: Event, instruction: Dict[str, Any]
    ) -> List[Event]:
        ...
        try:
            if action_type == "update_context_summary":
                ...
                synthesis_prompt = config.get(
                    "synthesis_prompt",
                    "Update the running summary with key information from this event.",
                )
                ...
                task_description = config.get(
                    "task_template", "Process event: {event_type}"
                )
...
"""
                ...
                logging.info(
                    f"Created Conseil task for instruction '{instruction.get('name')}' with complexity '{complexity}'"
                )
                ...
                notification_event = Event(
                    ...
                    metadata={
                        ...
                        "message": notification_config.get(
                            "message", "An event was processed"
                        ).format(
                            event_type=event.type,
                            timestamp=event.timestamp.isoformat(),
                            **event.metadata,
                ...
                email_event = EmailEvent(
                    ...
                    email_data={
                        ...
                        "subject": email_config.get(
                            "subject", f"Notification: {event.type}"
                        ),
                        "body": email_config.get(
                            "body_template", "Event processed: {event_type}"
                        ).format(
    ...
    async def _update_instruction_execution(self, user_id: str, instruction_id: str):
        """Update instruction execution count"""
        try:
            # Get the instruction document
            item = await self._storage.get_document(
                instruction_id, self.instruction_collection
            )
            if item:
                item["execution_count"] = item.get("execution_count", 0) + 1
                item["last_executed"] = datetime.utcnow().isoformat()
                await self._storage.upsert_document(item, self.instruction_collection)
        except Exception as e:
...
class TaskMonitorDriver(ToolDriver):
    ...
    def __init__(
        ...
        # Initialize provider-agnostic storage
        factory = get_provider_factory()
        runtime_config = get_runtime_config()
        self._storage = factory.create_storage_provider(runtime_config)
        ...
        self.task_collection = (
            config.get("task_collection", "tasks") if config else "tasks"
        )
    ...
    async def _create_task_record(self, task_event: WorkerTaskEvent) -> str:
        ...
        try:
            ...
            await self._storage.create_document(task_record, self.task_collection)
            return task_id
        except Exception as e:
    ...
    async def _update_task_status(self, task_id: str, status: str, user_id: str):
        ...
        try:
            task_record = await self._storage.get_document(
                task_id, self.task_collection
            )
            if task_record:
                ...
                await self._storage.upsert_document(task_record, self.task_collection)
        except Exception as e:
    ...
    async def _get_task_metrics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            query = {"filter": {"user_id": user_id}, "collection": self.task_collection}
            items = await self._storage.query_documents(query)
...
class SchedulerDriver(ToolDriver):
    ...
    def __init__(
        ...
        self.schedule_collection = (
            config.get("schedule_collection", "schedules") if config else "schedules"
        )
        ...
        try:
            ...
            schedule_record = {
                ...
                "next_trigger": self._calculate_next_trigger(
                    event.metadata.get("cron")
                ),
            }

            await self._storage.create_document(
                schedule_record, self.schedule_collection
            )
            return schedule_id
        except Exception as e:
    ...
    async def _update_schedule(
        self, schedule_id: str, user_id: str, updates: Dict[str, Any]
    ):
        """Update an existing schedule"""
        try:
            schedule_record = await self._storage.get_document(
                schedule_id, self.schedule_collection
            )
            if schedule_record:
                # Update fields
                if "cron" in updates:
                    schedule_record["cron"] = updates["cron"]
                    schedule_record["next_trigger"] = self._calculate_next_trigger(
                        updates["cron"]
                    )
                ...
                await self._storage.upsert_document(
                    schedule_record, self.schedule_collection
                )
        except Exception as e:
    ...
    async def _delete_schedule(self, schedule_id: str, user_id: str):
        """Delete a schedule"""
        try:
            await self._storage.delete_document(schedule_id, self.schedule_collection)
        except Exception as e:
    ...
    async def _trigger_scheduled_events(self) -> List[Event]:
        ...
        try:
            ...
            # Query for schedules that are due (simplified query for abstraction layer)
            query = {
                "filter": {"enabled": True},
            ...
            # Filter in memory for next_trigger <= current_time
            due_schedules = [
                item
                for item in items
                if item.get("next_trigger") and item.get("next_trigger") <= current_time
            ]
            ...
            for schedule in due_schedules:
                try:
                    ...
                    schedule["next_trigger"] = self._calculate_next_trigger(
                        schedule.get("cron")
                    )
                    ...
                    logging.error(
                        f"Failed to trigger schedule {schedule.get('id')}: {e}"
                    )
