...
from .drivers import DriverManifest, BaseDriver
...
async def initialize_all_drivers():
    ...
    logger.info(f"Driver initialization complete: {successful} successful, {failed} failed")
    
    if failed > 0:
        logger.warning(f"{failed} drivers failed to initialize - system may have reduced functionality")
...
def get_driver_definitions() -> List[Tuple[DriverManifest, Type[BaseDriver]]]:
    ...
    try:
        from .core_drivers import (
            ContextHubDriver, ChatAgentDriver, AuthenticationDriver
        )
        drivers.extend([
...
async def initialize_required_drivers():
    ...
    for manifest, driver_class in required_drivers:
        try:
            ...
            raise RuntimeError(f"Required driver {manifest.id} failed to initialize") from e
    
    logger.info(f"Required driver initialization complete: {len(required_drivers)} drivers")
