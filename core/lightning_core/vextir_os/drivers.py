...
class DriverRegistry:
    ...
    async def register_driver(self, manifest: DriverManifest, driver_class: Type[Driver], config: Optional[Dict[str, Any]] = None):
    ...
    async def start_driver(self, driver_id: str, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            instance = DriverInstance(driver=driver, manifest=manifest, status="starting")
    ...
    async def route_event(self, event: Event) -> List[Event]:
        ...
        for driver_id in capable_drivers:
            ...
            try:
                ...
                logging.error(f"Error in driver {driver_id} handling event {event.type}: {e}")
