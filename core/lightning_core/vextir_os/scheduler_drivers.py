...
from .drivers import Driver, ToolDriver, DriverManifest, DriverType, ResourceSpec, driver


@driver("cron_scheduler", DriverType.TOOL,
...
class CronSchedulerDriver(ToolDriver):
    ...
    def __init__(self, manifest: DriverManifest, config: Optional[Dict[str, Any]] = None):
        ...
        self.running = False
        self.check_interval = config.get("check_interval", 60) if config else 60  # Check every minute
        ...
        if event.type == "plan.schedule":
            ...
            result_event = Event(
                ...
                metadata={
                    ...
                    "scheduled_events": len([e for e in plan.get("events", []) if e.get("kind") == "time.cron"])
                }
            )
    ...
    async def _schedule_plan_events(self, plan: Dict[str, Any], user_id: str):
        ...
        for event_def in plan.get("events", []):
            if event_def.get("kind") == "time.cron":
                ...
                cron_expression = event_def.get("schedule", "0 20 * * *")  # Default to 8 PM daily
                
                # Create unique job ID
                job_id = f"{plan_name}_{event_name}_{user_id}"
                
                self._add_cron_job(job_id, cron_expression, event_name, user_id, {
                    "plan_name": plan_name,
                    "event_definition": event_def
                })
                
                logging.info(f"Scheduled cron job {job_id}: {cron_expression} -> {event_name}")
    
    def _add_cron_job(self, job_id: str, cron_expression: str, event_name: str, user_id: str, metadata: Optional[Dict] = None):
            ...
            try:
                ...
                for job_id, job_info in jobs_to_run:
                    try:
                        ...
                        logging.info(f"Executed job {job_id}, next run: {job_info['next_run']}")
    ...
    def _execute_job(self, job_id: str, job_info: Dict[str, Any]):
        ...
        try:
            ...
            logging.info(f"Emitted scheduled event {job_info['event_name']} for job {job_id}")
...
@driver("interval_scheduler", DriverType.TOOL,
...
class IntervalSchedulerDriver(ToolDriver):
    ...
    def __init__(self, manifest: DriverManifest, config: Optional[Dict[str, Any]] = None):
        ...
        self.scheduler_thread: Optional[threading.Thread] = None
        self.running = False
        self.check_interval = config.get("check_interval", 30) if config else 30  # Check every 30 seconds
    ...
    async def handle_event(self, event: Event) -> List[Event]:
        ...
        if event.type == "plan.schedule":
            ...
            result_event = Event(
                ...
                metadata={
                    "plan_name": plan.get("plan_name", "unknown"),
                    "scheduled_intervals": len([e for e in plan.get("events", []) if e.get("kind") == "time.interval"])
                }
            )
            output_events.append(result_event)
        ...
        for event_def in plan.get("events", []):
            if event_def.get("kind") == "time.interval":
                ...
                if interval_seconds:
                    ...
                    self._add_interval_job(job_id, interval_seconds, event_name, user_id, {
                        "plan_name": plan_name,
                        "event_definition": event_def
                    })
                    
                    logging.info(f"Scheduled interval job {job_id}: every {interval_seconds}s -> {event_name}")
    ...
    def _add_interval_job(self, job_id: str, interval_seconds: int, event_name: str, user_id: str, metadata: Optional[Dict] = None):
            ...
            try:
                ...
                for job_id, job_info in jobs_to_run:
                    try:
                        ...
                        # Update next run time
                        job_info["next_run"] = current_time + timedelta(seconds=job_info["interval_seconds"])
                        job_info["run_count"] += 1
                        job_info["last_run"] = current_time
                        
                        logging.info(f"Executed interval job {job_id}, next run: {job_info['next_run']}")
    ...
    def _execute_job(self, job_id: str, job_info: Dict[str, Any]):
        ...
        try:
            ...
            logging.info(f"Emitted scheduled interval event {job_info['event_name']} for job {job_id}")
