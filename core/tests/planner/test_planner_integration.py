...
from lightning_core.planner.validator import validate_plan_new, run_validations_parallel
...
class TestPlannerIntegration:
    ...
    def test_01_simple_summarization_workflow(self, mock_client):
        ...
        mock_client.return_value.chat.completions.create.return_value = self._mock_openai_response(expected_plan)
    ...
    def test_03_vex_agent_phone_call_workflow(self, mock_client):
        ...
        assert any(step["action"] == "chat.sendTeamsMessage" for step in result["steps"])
    ...
    def test_04_scheduled_llm_analysis_workflow(self, mock_client):
        ...
        assert any(step["action"] == "event.schedule.create" for step in result["steps"])
    ...
    def test_10_complex_multi_stage_workflow(self, mock_client):
        ...
        expected_actions = {
            "event.schedule.create", "agent.conseil", "llm.general_prompt", 
            "agent.vex", "llm.summarize", "chat.sendTeamsMessage"
        }
    ...
    def test_tool_registry_completeness(self):
        ...
        expected_tools = {
            "agent.conseil", "agent.vex", "llm.summarize", "llm.general_prompt",
            "chat.sendTeamsMessage", "event.schedule.create", "event.timer.start"
        }
        
        actual_tools = set(tools.keys())
        assert actual_tools == expected_tools, f"Tool registry mismatch: {actual_tools - expected_tools}"
    ...
    def test_event_registry_integration(self):
        ...
        expected_external = {
            "event.email.check", "event.calendar.sync", 
            "event.webhook.github", "event.manual.trigger"
        }
        
        actual_external = set(external_events.keys())
        assert expected_external.issubset(actual_external), f"Missing external events: {expected_external - actual_external}"
