...
class TestPlanModel:
    ...
    def test_valid_plan(self):
        ...
        events = [
            ExternalEventModel(name="event.daily", kind="time.cron", schedule="0 9 * * *")
        ]
        steps = [
            StepModel(name="step1", on=["event.daily"], action="test.action")
        ]
    ...
    def test_reactive_plan(self):
        ...
        events = [
            ExternalEventModel(name="event.webhook", kind="webhook")
        ]
        steps = [
            StepModel(name="step1", on=["event.webhook"], action="process.webhook", emits=["event.processed"]),
            StepModel(name="step2", on=["event.processed"], action="notify.user")
        ]
        
        plan = PlanModel(
            plan_name="reactive_plan",
            graph_type="reactive",
            events=events,
            steps=steps
        )
