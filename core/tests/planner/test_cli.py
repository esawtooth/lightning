...
class TestPlannerCLI:
            ...
            "steps": [
                {
                    ...
                    "args": {"text": "Sample text to summarize", "style": "brief"},
    ...
    def test_generate_mermaid_diagram_file_not_found(self):
        ...
        result = generate_mermaid_diagram(
            "nonexistent.json", "output.md", verbose=False
        )
        assert result is False
    ...
    def test_mermaid_diagram_content_structure(self):
        ...
        try:
            ...
            assert any(
                "event_manual_trigger" in line and "manual" in line for line in lines
            )
            assert any(
                "style event_manual_trigger fill:#e1f5fe" in line for line in lines
            )

            # Check for steps with styling
            assert any(
                "summarize_text" in line and "llm.summarize" in line for line in lines
            )
            ...
            assert any(
                "style event_task_complete fill:#f3e5f5" in line for line in lines
            )

            # Check for connections
            assert any(
                "event_manual_trigger --> summarize_text" in line for line in lines
            )
            assert any(
                "summarize_text --> event_task_complete" in line for line in lines
            )
        ...
        complex_plan = {
            ...
            "steps": [
                ...
                },
                {
                    "name": "create_summary",
                    "action": "llm.summarize",
                    "args": {"text": "Research findings", "style": "executive"},
                ...
                },
                {
                    ...
                    "args": {"channel_id": "general", "content": "Summary ready"},
