###############################################################################
# vextir infra – Container Apps Migration
# 2025-06-24
###############################################################################
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. SIMPLIFIED NETWORKING (Container Apps manages most networking)
# ─────────────────────────────────────────────────────────────────────────────
...
ca_subnet = network.Subnet(
    ...
    address_prefix="10.0.0.0/23",  # Container Apps needs /23 or larger
    delegations=[
        network.DelegationArgs(
            name="containerAppsDeleg",
            service_name="Microsoft.App/environments",
        )
    ],
)

# Private endpoint subnet for Cosmos DB
...
# Function app role assignment and deployment (same as before)
...
# Function app ZIP deployment (unchanged)
def build_zip():
...
if runtime.is_dry_run():
    zip_asset = pulumi.StringAsset("")
else:
    zip_path  = build_zip()
    zip_asset = pulumi.FileAsset(zip_path)
...
# Helper function to create container apps
...
# Conseil Container App (internal service)
conseil_secrets = [
    app.SecretArgs(name="openai-api-key", value=openai_api_key),
]

conseil_ca = create_container_app(
    "conseil",
    conseil_image,
    8080,
    [
        app.EnvironmentVarArgs(name="OPENAI_API_KEY", secret_ref="openai-api-key"),
        app.EnvironmentVarArgs(name="NODE_ENV", value="production"),
        app.EnvironmentVarArgs(name="CONSEIL_MODE", value="server"),
    ],
    secrets=conseil_secrets,
    min_replicas=0,  # Scale to zero when not in use
    max_replicas=5,
    external=False,  # Internal service
    health_path="/",  # Conseil doesn't have /health endpoint
)

# Get Conseil internal URL
conseil_internal_url = pulumi.Output.concat("https://", conseil_ca.name, ".", ca_env.default_domain)

# Function app settings (updated with Container Apps URLs)
func_settings = {
    ...
    "CONSEIL_URL": conseil_internal_url,
...
# ─────────────────────────────────────────────────────────────────────────────
# 15. FRONT DOOR + DNS (Updated for Container Apps)
# ─────────────────────────────────────────────────────────────────────────────
...
# Routes
def afd_route(name, pattern, og, origin, cd):
    cdn.route.Route(
        f"{name}-route",
        resource_group_name=rg.name,
        profile_name=fd_profile.name,
...
afd_route("ui",    "/*",          ui_og,    ui_origin,    ui_cd)
afd_route("api",   "/api/*",      api_og,   api_origin,   api_cd)
afd_route("voice", "/voice-ws/*", voice_og, voice_origin, voice_cd)
afd_route("hub",   "/*",          hub_og,   hub_origin,   hub_cd)
...
# Front Door verification TXT records
...
# Function app custom domain binding
...
pulumi.export("conseilInternalUrl", conseil_internal_url)

# Public URLs (via Front Door)
